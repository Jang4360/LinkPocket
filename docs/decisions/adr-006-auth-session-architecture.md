...
**관찰 지표**: 재인증 주기가 유발하는 마찰(재인증 완료율·이탈률). 주기 값(예: 7일/14일/30일)은 아직 미정.

**결과 (미실행 — 다음에 확정할 것)**
- 재인증 주기 값을 아직 정하지 않았다.
- 이미 머지된 rotation/reuse-detection 구현(device_session/refresh_token 2테이블)을 되돌리는 작업 범위를 아직 확정하지 않았다.
- 실행 여부·시점은 별도 논의 후 결정한다.
