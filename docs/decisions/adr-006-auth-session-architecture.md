...
> ⚠️ **2026-07-26 재검토로 뒤집힘 — 실행은 보류 상태.** 아래 원래 결정(2026-07-16)은 plan-01에서 이미 구현·머지됐다. 회고에서 사람이 반대 방향(주기적 강제 재인증)으로 재결정했으나, 이미 머지된 인증 흐름 전체를 되돌리는 작업이라 범위·주기 값 확정 전까지 실행하지 않는다. 새 결정은 맨 아래 "2026-07-26 재검토" 절 참고.
...
### 2026-07-26 재검토 (사람 주도 문제 재정의)

**문제 정의 (사람 답변 그대로)**
1. 적어도 구글 계정의 refresh token이 유지되는 시간만큼은 로그인 상태를 보장해야 한다고 본다.
2. 로컬 기기(브라우저 확장 저장소)가 털리는 시나리오에서, "이 서비스 접근 권한" 탈취보다 "개인정보가 포함된 계정 전체" 탈취가 비교가 안 되게 더 치명적이다 — 반드시 막아야 한다.
3. 처음부터 일정 주기마다 매번 재인증으로 가야 한다.

**가설**
로컬에 장기 자격증명(refresh token)을 두면, 탈취됐을 때 단순 "이 앱 접근권" 이상의 위험(2번 답변이 우려한 "개인정보 포함 계정 전체")으로 번질 수 있다. 로컬에 장기 자격증명을 아예 안 두는 쪽(주기적 강제 재인증)이 근본적으로 더 안전하다.

**대안**
| 선택지 | 트레이드오프 |
|---|---|
| `chrome.storage.local` + rotation(기존, 이미 구현됨) | 재로그인 최소화. 로컬 탈취 시 rotation/reuse detection이 완화책이지만 탈취 자체는 막지 못함 |
| **일정 주기마다 강제 재인증** (신규 채택) | 로컬에 장기 자격증명을 두지 않아 탈취 표면 자체가 줄어든다. 주기마다 실제 재로그인 마찰 발생 |

**선택 (사람 판단)**: 일정 주기마다 강제 재인증. 재로그인 마찰보다 "로컬 탈취 시 계정 전체가 위험해질 수 있다"는 우려가 결정적이었다.

**관찰 지표**: 재인증 주기가 유발하는 마찰(재인증 완료율·이탈률).

### 2026-07-27 확정 — 실행 범위 재정의

회고를 이어가며 명확해진 것: "rotation을 버리고 주기적 재인증으로 대체"가 아니라, **rotation/reuse-detection(탈취 감지)은 그대로 두고 그 위에 14일 고정 만료(로그인 유지 기간 상한)를 얹는다.** 이 둘은 서로 다른 문제를 푼다 — rotation은 "토큰이 복제돼 두 곳에서 쓰이는가"를 감지하고, 고정 만료는 "얼마나 오래 로그인 상태를 유지할지"를 정한다.

- **주기 값: 14일, 고정 만료**(슬라이딩 아님 — 쓸 때마다 연장되면 사실상 영구 로그인과 같아져 탈취 방어 효과가 약해진다). OAuth 2.0 Security BCP 권고 범위(민감 앱 7~30일, 브라우저 기반은 더 짧게) 안에서, 마찰과 보안의 균형으로 선택.
- **적용 지점**: `device_session.created_at` 기준. 그로부터 14일이 지나면 refresh token이 재사용된 게 아니어도(정상 회전 중이어도) 거부하고 `AUTH_REFRESH_TOKEN_EXPIRED`(401, 신설)로 family 전체를 폐기한다 — reuse 폐기와 같은 효과(전체 재인증 필요), 사유 코드만 다르다(사용자에게 "탈취 의심" 경고 대신 "세션 만료" 안내를 보여주기 위함).
- **14일 이내는 기존 rotation 계약이 그대로 유지된다** — 회귀 없음.

**결과**: `test/01c-refresh-fixed-expiry` 계약 테스트 3개 작성, red 확인(구현 전). 기존 `TokenRotationContractTest` 회귀 없음 확인.
...
- 결정 3은 원래(2026-07-16) 이 ADR에서 유일하게 보안보다 사용성을 우선한 지점이었다. 2026-07-26 재검토로 그 우선순위가 뒤집혀, 지금은 오히려 **가장 보안 우선으로 재조정된 지점**이다 — 단, 실행(구현 되돌리기)은 아직 보류 상태다.

## 재검토 조건

- ~~실제 사용자 피해 사례(익스텐션 토큰 탈취)가 관측되면 `chrome.storage.session`으로 전환한다.~~ → 2026-07-26 재검토로 선제적으로 결정됨(피해 관측을 기다리지 않기로 함). 실행 범위·주기 값 확정이 다음 단계.
