...
| exp-06 · 수집기 SSRF 안전성 회귀(우회 corpus 기반) | 로컬 | **완료(2026-07-27)** — 실험이 아니라 긴급 보안 수정으로 전환됨(아래 참고) | D(외부 수집) 보안. ADR-011이 이 프로젝트에서 가장 넓은 공격 표면이라 명시한 지점 |
...
**exp-06 결과 요약**: corpus를 설계하며 `LinkFetchServiceImpl` 코드를 재검토하다가 실험 없이도 코드만으로 확정할 수 있는 보안 결함 2건(DNS rebinding TOCTOU, IPv6 미검사)을 발견해, "측정"이 아니라 **긴급 보안 수정**으로 바로 전환했다([PR #31](https://github.com/Jang4360/LinkPocket/pull/31), [mistake-ledger](../operations/mistake-ledger.md) `adr-implementation-drift` 2회째). 두 결함 모두 `LinkFetchSsrfAdvancedContractTest`(영구 계약 테스트)로 회귀 방지 처리했다 — exp-06이 원래 계획한 "corpus 기반 회귀 검증"의 목적은 이 계약 테스트로 달성됐다고 본다.
